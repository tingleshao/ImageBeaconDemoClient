...
//        });
        ...
        if (doneGettingData) {
            ...
            if (decodeTri) {
                // decode triangle
                int[] res = this.decodeByteArrayTri("");
                Canvas canvas = new Canvas(bmp);
                Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        //        paint.setColor(Color.YELLOW);
         //       canvas.drawCircle(5, 5, 10, paint);
                this.drawMesh(canvas, paint, res);
            }
            imageView.setImageBitmap(bmp);
            stopScan();
            reset();
    ...
    private void drawMesh(Canvas canvas, Paint paint, int[] res) {

        paint.setStrokeWidth(2);
        paint.setColor(android.graphics.Color.RED);
        paint.setStyle(Paint.Style.FILL_AND_STROKE);
        paint.setAntiAlias(true);
        int numberOfTriangles = res.length / 9;
        int shift = numberOfTriangles * 6;
        for (int i = 0; i < numberOfTriangles; i ++ ) {
            int x1 = res[i*6] + 128;
            int y1 = res[i*6+1]+ 128;
            int x2 = res[i*6+2]+ 128;
            int y2 = res[i*6+3]+ 128;
            int x3 = res[i*6+4]+ 128;
            int y3 = res[i*6+5]+ 128;

            Point point1_draw = new Point(x1, y1);
            Point point2_draw = new Point(x2, y2);
            Point point3_draw = new Point(x3, y3);
            int colorR = res[shift+i*3]+ 128;
            int colorG = res[shift+i*3+1]+ 128;
            int colorB = res[shift+i*3+2]+ 128;
            paint.setColor(android.graphics.Color.argb(255, colorR*8, colorG*8, colorB*8));
...
//canvas.drawLine(point1_draw.x,point1_draw.y,point2_draw.x,point2_draw.y, paint);
    ...
    public int[] decodeByteArrayTri(String inputString) {
        byte[] result;
        if (!decodeColor) {
            if (!decodeTri) {
                result = new byte[4096];
      ...
      //  int n = (int)result[0];
        ...
        String resStr = "";
   //     int nOfTriangles = resultLength / 9;
        for (int i = 0; i < resultLength; i ++) {
         //   if (result[i] < 127) {
                resStr = resStr + String.valueOf(result[i]) + " ";
            res[i] = result[i];
        //    }
     //       else {
